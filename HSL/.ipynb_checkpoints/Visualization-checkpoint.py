...
def preprocessing(json_directory):
        
        df = pd.DataFrame(columns=['JSON'])
        # find json files
...
#             df = df.drop(df.index[-1])
        ...
        master_array = np.empty((len(sorted_json_files),21,2), dtype='float64')
        for index, row in df.iterrows():
            coo, cf = handshape(row['JSON']).get_right_fingers_from_json
            master_array[index] = coo
                
        train_dataset = master_array.reshape((len(sorted_json_files),21*2))
        ...
        return(df,sorted_json_files, train_dataset, principalDf)
...
#         self._frame_directory = frame_directory
        ...
        self._df,self._sorted_json_files, self._train_dataset, self._principalDf = preprocessing(self._directory)
